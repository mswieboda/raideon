...
@export var hover_color : Color = Color.GREEN: set = set_hover_color
@export var font_size : int = 16: set = set_font_size
@export var hoverable : bool = true: set = set_hoverable
...
func set_font_size(size):
  if size != font_size:
    font_size = size
    #var dfont = get_theme_default_font() #.duplicate(true)
    #dfont.size = font_size
    #add_theme_font_override("font", dfont)


func change_color(new_color : Color):
  set("theme_override_colors/font_color", new_color)
